...
class Wordle:
    ...
    def get_or_check_random_word(*inputted_word):
        ...
        if len(inputted_word) == 0: 
            return random.choice(random_words).upper()
    ...
    def input_letter(self, letter):
        try:
            ...
            if len(self.players_input_list) >= 5 or not self.validate_input(letter): 
    ...
    def remove_letter(self, letter):
        ...
        letter = chr(letter).upper()
        if len(self.players_input_list) == 0: 
            self.show_error_message("There is no input")
        else:
    ...
    def check_winner(self):
        ...
        if len(self.players_input_list) != 5 or self.get_or_check_random_word(player_word) == False: 
            self.show_error_message("Not a 5 letter word")
        else:
        # making copies of the lists
            input_list_copy = self.players_input_list.copy()
            answer_list_copy = self.word_of_the_day_list.copy()
            list_of_current_colours = [GREY] * 5
            # this check if letter in correct position
            for i in range(5):
                if input_list_copy[i] == answer_list_copy[i]:
                    list_of_current_colours[i] = GREEN
                    answer_list_copy[i] = None
                    input_list_copy[i] = None
            # this check if they are in the list thingy
            for i in range(5):
                if input_list_copy[i] is not None and input_list_copy[i] in answer_list_copy:
                    list_of_current_colours[i] = YELLOW
                    answer_list_copy[answer_list_copy.index(input_list_copy[i])] = None
            for i in range(5):
            ...
            if event.type == pygame.QUIT:
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_BACKSPACE:
                        game.remove_letter(event.key)
                    elif event.key == pygame.K_RETURN:
                        game.check_winner()
                        
                    else:
                        game.input_letter(event.key)      
                else:
                    continue
